...
NS_LOG_COMPONENT_DEFINE ("MyApps");
...
Sender::GetTypeId (void)
{
  static TypeId tid = TypeId ("Sender")
    ...
    .AddAttribute ("Interval", "Delay between transmissions.",
                   StringValue ("ns3::ConstantRandomVariable[Constant=2s]"),
...
void Sender::StartApplication ()
{
  ...
  if (m_socket == 0) {
      ...
      if(m_broadcast)
      {
    	  InetSocketAddress remote = InetSocketAddress (Ipv4Address ("255.255.255.255"), 1603);
...
Receiver::StartApplication ()
{
  ...
  std::stringstream ss;
  uint32_t id=m_socket->GetNode()->GetId();;
  ss<<"application"<<id<<std::endl;
  ...
  while ((packet = socket->RecvFrom (from))) {
      ...
      if (packet->FindFirstMatchingByteTag (timestamp)) {
          Time tx = timestamp.GetTimestamp ();
              ...
              delay=Simulator::Now () - tx;
              //m_delay->Update (delay);
              m_delayTotal+=delay.GetNanoSeconds();

              NS_LOG_INFO ("Received " << packet->GetSize () << " bytes from " <<
                                InetSocketAddress::ConvertFrom (from).GetIpv4 ()<<"delay"<<delay);
        }
...
Receiver::CheckData ()//CSV data
{
	  ...
	  out << (Simulator::Now ()).GetSeconds () <<","<< kbs<<","<< m_recvPackets << ","<<delayone<< std::endl;

	  out.close ();
	  m_recvPackets = 0;
	  m_delayTotal=0;
	  Simulator::Schedule (Seconds (150.0), &Receiver::CheckData, this);
}
