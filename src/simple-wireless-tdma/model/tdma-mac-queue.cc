...
bool
TdmaMacQueue::Enqueue (Ptr<const Packet> packet, const TdmaHeader &hdr)
{
...
Ptr<const Packet>
TdmaMacQueue::Dequeue (TdmaHeader *hdr)
{
  NS_LOG_FUNCTION_NOARGS ();
  Cleanup ();
  if (!m_queue.empty ())
...
Ptr<const  Packet>
TdmaMacQueue::Peek (TdmaHeader *hdr)
{
  NS_LOG_FUNCTION_NOARGS ();
  Cleanup ();
  if (!m_queue.empty ())
...
Mac48Address
TdmaMacQueue::GetAddressForPacket (PacketQueueI it)
{
	return it->hdr.GetDest ();
