...
void
TdmaMacLow::SetRxCallback (Callback<void,Ptr<Packet>,const TdmaHeader *> callback)
{
...
TdmaMacLow::Receive (Ptr<Packet> packet)
{
  ...
  NS_LOG_DEBUG ("rx group from=" << hdr.GetSrc());
  WifiMacTrailer fcs;
...
uint32_t
TdmaMacLow::GetSize (Ptr<const Packet> packet, const TdmaHeader *hdr) const
{
  WifiMacTrailer fcs;
...
void
TdmaMacLow::ForwardDown (Ptr<const Packet> packet, const TdmaHeader* hdr)
{
  NS_LOG_DEBUG (" to=" << hdr->GetDest() <<
                ", size=" << packet->GetSize ());
