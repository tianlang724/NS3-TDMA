...
public:
  ...
  virtual void Enqueue (Ptr<const Packet> packet, TdmaHeader hdr,TdmaTag tag);
  ...
  virtual void SetTdmaControllerDynamic (Ptr<TdmaControllerDynamic> controller);
  virtual Ptr<TdmaControllerDynamic> GetTdmaControllerDynamic (void) const;
  ...
  virtual void StartRouteTransmission (uint64_t transmissionTime,Ptr<Packet> packet);
   ...
   * can be sent safely.
   */
  void Queue (Ptr<const Packet> packet, const TdmaHeader &hdr);
...
private:
  void Receive (Ptr<Packet> packet, const TdmaHeader *hdr);
  void ForwardUp (Ptr<Packet> packet, Mac48Address from, Mac48Address to);
  void TxOk (const TdmaHeader &hdr);
  void TxFailed (const TdmaHeader &hdr);
  ...
  void SendRouteDown (Time remainingTime,Ptr<Packet>packet,TdmaHeader *hdr);
  uint8_t GetNewSeq();
  void CheckData();
  ...
  std::deque<TdmaHeader> m_recvPacketsQueue;  //接受缓冲队列
  uint32_t m_recvQueueMaxSize;  //接受缓冲队列的最大长度
  ...
  uint8_t m_seq;
  uint32_t m_nodeId;
  uint32_t m_recvPackets;
  uint32_t m_ownPackets;
  uint32_t m_repeatePackets;
  uint32_t m_ttlPackets;
  uint32_t m_bytesTotal;
  uint32_t m_delayTotal;
  uint32_t m_sendPackets;
  std::string m_CSVfileName;
  bool m_isTdmaRunning;

  Ptr<RateErrorModel> m_rem;
