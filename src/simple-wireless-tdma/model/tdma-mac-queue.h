...
public:
  ...
  bool Enqueue (Ptr<const Packet> packet, const TdmaHeader &hdr);
  /**
   * \brief Dequeues a packet based on the header passed to it
   *
   * \param hdr header to be dequeued from the TdmaMacQueue
   */
  Ptr<const Packet> Dequeue (TdmaHeader *hdr);
  Ptr<const Packet> Peek (TdmaHeader *hdr);
  /**
...
private:
  ...
  void Cleanup (void);
  Mac48Address GetAddressForPacket (PacketQueueI);
