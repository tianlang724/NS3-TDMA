...
private:
  ...
  double m_totalTime;
  std::string m_rate;
  std::string m_phyMode;  //wifi配置
  uint32_t m_nodeSpeed;   //移动模型移动速度
...
private:
  ...
  void InstallInternetStackDsdv ();
  void InstallInternetStackAodv ();
  void InstallInternetStackDsr ();
  void InstallInternetStackOlsr ();
  void InstallApplications ();
  void InstallMyApplications();
  void SetupMobility ();
  void SetupMobilityGrid ();
...
int main (int argc, char **argv)
{
  ...
  std::string rate ("70kbps"); //OnOffApplication  rate
  ...
  cmd.Parse (argc, argv);
  LogComponentEnable("TdmaExample",LOG_LEVEL_ALL);
  //LogComponentEnable("TdmaCentralMac",LOG_LEVEL_DEBUG);
  ///LogComponentEnable("TdmaController",LOG_LEVEL_DEBUG);
  //LogComponentEnable("TdmaMacLow",LOG_LEVEL_DEBUG);
  //LogComponentEnable("TdmaNetDevice",LOG_LEVEL_DEBUG);
  //LogComponentEnable("SimpleWirelessChannel",LOG_LEVEL_DEBUG);  //watch!!!
  //LogComponentEnable("TdmaMacQueue",LOG_LEVEL_DEBUG);
  //LogComponentEnable("TdmaControllerDynamic",LOG_LEVEL_DEBUG);
  //LogComponentEnable("TdmaRouteTable",LOG_LEVEL_DEBUG);
  //LogComponentEnable("MyApps",LOG_LEVEL_ALL);
  ...
  test.CaseRun (nWifis, nSinks, totalTime, rate, phyMode, nodeSpeed, periodicUpdateInterval,
               settlingTime, dataStart,CSVfileName,usingWifi,txpDistance, nSlots, slotTime, gaurdTime, interFrameGap,pauseTime);
  ...
  while(packet=socket->Recv())
    {
	  ...
	  NS_LOG_UNCOND("packet time"<<tag.GetSTimeStamp());
	  //显示包的源和目的

	 // Node node=socket->GetNode();
      bytesTotal += packet->GetSize ();
      delayTotal+=delay.GetNanoSeconds();
      packetsReceived += 1;
      Address addr;
      socket->GetSockName (addr);
      InetSocketAddress iaddr = InetSocketAddress::ConvertFrom (addr);
      NS_LOG_UNCOND (Simulator::Now ().GetSeconds ()<<" Received one packet! socket:"
    		  <<iaddr.GetIpv4 () << " port: " << iaddr.GetPort ()<<" delay: "<<delay);
    }
}

void
TdmaExample::CheckThroughput ()//CSV data
{
  double kbs = (bytesTotal * 8.0) / 1000;
  uint64_t delayone;
  bytesTotal = 0;
  if(packetsReceived>0)
	  delayone=delayTotal/packetsReceived;
  else
	  delayone=0;
  std::ofstream out (m_CSVfileName.c_str (), std::ios::app);

  out << (Simulator::Now ()).GetSeconds () << "," << kbs << "," << packetsReceived << ","<<delayone<<"," << m_nSinks << std::endl;
...
TdmaExample::CaseRun (uint32_t nWifis, uint32_t nSinks, double totalTime, std::string rate,
                      ...
                      uint32_t slotTime, uint32_t gaurdTime, uint32_t interFrameGap,uint32_t pauseTime)
{
  m_nWifis = nWifis;
  m_nSinks = nSinks;
  ...
  AnimationInterface anim ("tdma-animation.xml");
  //必须放在安装了应用层之后
	//AnimationInterface::SetConstantPosition (nodes.Get (1), 10, 30);
	//AnimationInterface::SetConstantPosition (nodes.Get (2), 10, 33);
	/*uint32_t i=0;
	for(i=0;i<nodes.GetN();i++)
	{
		anim.UpdateNodeDescription(nodes.Get(i),"User");
		anim.UpdateNodeColor (nodes.Get (i), 0, 255, 0);
	}
	anim.EnablePacketMetadata (true);
	anim.EnableIpv4RouteTracking ("tdma-routingtable.xml", Seconds (0), Seconds (5), Seconds (0.25)); //Optional
...
*/
  ...
  Simulator::Run ();
  flowMonitor->SerializeToXmlFile("Tdma.xml", false, false);
  Simulator::Destroy ();
}
...
TdmaExample::SetupMobility ()
{
  ...
  pos.Set ("X", StringValue ("ns3::UniformRandomVariable[Min=-1000.0|Max=2000.0]"));
  pos.Set ("Y", StringValue ("ns3::UniformRandomVariable[Min=-1000.0|Max=2000.0]"));
...
void
TdmaExample::SetupMobilityGrid()
{
	 MobilityHelper mobility;
	 mobility.SetPositionAllocator ("ns3::GridPositionAllocator",
	                                 "MinX", DoubleValue (0.0),
	                                 "MinY", DoubleValue (0.0),
	                                 "DeltaX", DoubleValue (700),
	                                 "DeltaY", DoubleValue (700),
	                                 "GridWidth", UintegerValue (m_nWifis),
...
TdmaExample::CreateDevices (std::string tr_name, bool usingWifi, double txpDistance)
{
  if (usingWifi)
  ...
  else
    {
      ...
      //TdmaHelper tdma = TdmaHelper (); // in this case selected, numSlots = nodes
...
void
TdmaExample::InstallInternetStackDsdv ()
{
...
void
TdmaExample::InstallInternetStackAodv()
{
	AodvHelper aodv;
...
void
TdmaExample::InstallInternetStackDsr()
{
...
TdmaExample::InstallApplications ()
{
  ...
  for (uint32_t clientNode = 0; clientNode <= m_nWifis - 1; clientNode++ )
    {
	  //设置一个节点为跟踪节点，则这个节点不发送数据，其他节点给这个节点发送，发送时间为1s
      for (uint32_t j = 0; j <= m_nSinks - 1; j++ )
        {
          ...
          Ptr<ConstantRandomVariable> var=CreateObject<ConstantRandomVariable>();
...
void
TdmaExample::InstallFlowMonitor()
{

}
void
TdmaExample::InstallMyApplications()
{
	uint32_t j;
	//uint32_t clientNode;
	for (j = 0; j <= m_nSinks - 1; j++ )
	{
		...
		//Ipv4Address sinkAddress=appSink->GetObject<Ipv4> ()->GetAddress (1, 0).GetLocal ();
		Ptr<Node> appSource = NodeList::GetNode (j);
		Ptr<Sender> sender = CreateObject<Sender>();
		sender->SetBroacast();
		appSource->AddApplication (sender);
		Ptr<UniformRandomVariable> var=CreateObject<UniformRandomVariable>();
		double time=var->GetValue(0,100);
		sender->SetStartTime (Seconds (time));

		/*
		for (clientNode = 0; clientNode <= m_nWifis - 1; clientNode++ )
		{
			if (j != clientNode)
	        {
				Ptr<Node> appSource = NodeList::GetNode (clientNode);
	        	Ptr<Sender> sender = CreateObject<Sender>();

	        	appSource->AddApplication (sender);
	            Ptr<UniformRandomVariable> var=CreateObject<UniformRandomVariable>();
	        	double time=var->GetValue(0,2);
	        	sender->SetStartTime (Seconds (time));
	        	std::ostringstream path;
	        	path <<"/NodeList/"<< clientNode<<"/ApplicationList/"<<j<<"$Sender/Destination";
	        	Config::Set (path.str(),Ipv4AddressValue(sinkAddress));
